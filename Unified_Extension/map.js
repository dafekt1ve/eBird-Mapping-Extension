(async function () {
  ...
  // Function to check if libraries are already loaded
  function checkLibraries() {
    return window.L && window.d3;
  }
  ...
  function renderMap(daysBack = 7) {
    ...
    document.querySelectorAll('.Observation').forEach(entry => {
      ...
      if (speciesElement && locationName && checklistLink && !isNaN(lat) && !isNaN(lng) && isRecent(dateTimeText, daysBack)) {
        sightings.push({
    ...
    // First, parse dateTime and sort sightings in DESCENDING order (most recent first)
    sightings.sort((a, b) => new Date(b.dateTime) - new Date(a.dateTime));
    ...
    sightings.forEach(({ species, lat, lng, locationName, checklistUrl, dateTime }) => {
      ...
      const speciesList = locationMap.get(key).speciesList;
      // Only set if this species hasn't been added yet (i.e. this is the most recent)
      if (!speciesList.has(species)) {
        speciesList.set(species, { url: checklistUrl, dateTime });
      }
    });
